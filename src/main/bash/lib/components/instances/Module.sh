...
function Module() {
    ...
    if [[ ! -n "${FW_ELEMENT_MDS_REQOUT_MDS["${id1}"]:-}" ]]; then
        FW_ELEMENT_MDS_REQOUT_MDS["${id1}"]+="${id2} "; writeSlow=true
    else
        case "${FW_ELEMENT_MDS_REQOUT_MDS["${id1}"]}" in
            *"${id2} "*)    Report application error "${FUNCNAME[0]}" "${1} requires ${3} ${4}" E904 "${id2}" "${3} ${id1}"; return ;;
            *)              FW_ELEMENT_MDS_REQOUT_MDS["${id1}"]+="${id2} "; writeSlow=true ;;
        esac
    fi

    if [[ "${writeSlow}" ]]; then FW_ELEMENT_MDS_REQOUT_COUNT[${id1}]=$(( FW_ELEMENT_MDS_REQOUT_COUNT[${id1}] + 1 )); fi
