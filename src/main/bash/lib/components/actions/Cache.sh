...
function Cache() {
    ...
    local id dir file sedFile array name errno themeFile
    ...
    shift; case "${cmd1}" in
        ...
        formats)
            ...
            if [[ -w "${file}" ]]; then
                ...
                for array in FW_OBJECT_FMT_LONG; do declare -p ${array} >> ${sedFile}; echo "" >> ${sedFile}; done
        ...
        levels)
            ...
            if [[ -w "${file}" ]]; then
                rm ${file}; sedFile="${file}-sed"
                for array in FW_OBJECT_LVL_LONG FW_OBJECT_LVL_CHAR_ABBR FW_OBJECT_LVL_STRING_THM; do declare -p ${array} >> ${sedFile}; echo "" >> ${sedFile}; done
                sed -e "s/declare -A/declare -A -g/g" ${sedFile} > ${file}; rm ${sedFile}
            fi ;;
        ...
        messages)
            ...
            if [[ -w "${file}" ]]; then
                rm ${file}; sedFile="${file}-sed"
                for array in FW_OBJECT_MSG_LONG FW_OBJECT_MSG_TYPE FW_OBJECT_MSG_CAT FW_OBJECT_MSG_ARGS FW_OBJECT_MSG_TEXT; do declare -p ${array} >> ${sedFile}; echo "" >> ${sedFile}; done
                sed -e "s/declare -A/declare -A -g/g" ${sedFile} > ${file}; rm ${sedFile}
            fi ;;
        ...
        modes)
            ...
            if [[ -w "${file}" ]]; then
                rm ${file}; sedFile="${file}-sed"
                for array in FW_OBJECT_MOD_LONG; do declare -p ${array} >> ${sedFile}; echo "" >> ${sedFile}; done
                sed -e "s/declare -A/declare -A -g/g" ${sedFile} > ${file}; rm ${sedFile}
            fi ;;

        module)
            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString1}" E801 1 "$#"; return; fi
            id="${1}"; if [[ "${id}" == "☰API☷" || "${id}" == "⫷Framework⫸" ]]; then Report process error "${FUNCNAME[0]}" "${cmd1}" E828 "module" "written"; return; fi
...
##TODO: Error message
            ...
            if [[ "${FW_ELEMENT_APP_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_APP_LONG[@]}; do
                    if [[ "${FW_ELEMENT_APP_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_APP_LONG[\"${elemId}\"]=\""${FW_ELEMENT_APP_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_APP_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_APP_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_DEP_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_DEP_LONG[@]}; do
                    if [[ "${FW_ELEMENT_DEP_DECMDS[${elemId}]}" == "${id}" ]]; then
            ...
            if [[ "${FW_ELEMENT_DIR_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_DIR_LONG[@]}; do
                    if [[ "${FW_ELEMENT_DIR_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_DIR_LONG[\"${elemId}\"]=\""${FW_ELEMENT_DIR_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_DIR_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_DIR_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_DLS_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_DLS_LONG[@]}; do
                    if [[ "${FW_ELEMENT_DLS_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_DLS_LONG[\"${elemId}\"]=\""${FW_ELEMENT_DLS_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_DLS_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_DLS_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_FIL_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_FIL_LONG[@]}; do
                    if [[ "${FW_ELEMENT_FIL_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_FIL_LONG[\"${elemId}\"]=\""${FW_ELEMENT_FIL_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_FIL_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_FIL_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_FLS_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_FLS_LONG[@]}; do
                    if [[ "${FW_ELEMENT_FLS_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_FLS_LONG[\"${elemId}\"]=\""${FW_ELEMENT_FLS_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_FLS_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_FLS_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_PAR_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_PAR_LONG[@]}; do
                    if [[ "${FW_ELEMENT_PAR_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_PAR_LONG[\"${elemId}\"]=\""${FW_ELEMENT_PAR_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_PAR_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_PAR_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_PRJ_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_PRJ_LONG[@]}; do
                    if [[ "${FW_ELEMENT_PRJ_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_PRJ_LONG[\"${elemId}\"]=\""${FW_ELEMENT_PRJ_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_PRJ_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_PRJ_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_SCN_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_SCN_LONG[@]}; do
                    if [[ "${FW_ELEMENT_SCN_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_SCN_LONG[\"${elemId}\"]=\""${FW_ELEMENT_SCN_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_SCN_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_SCN_DECMDS["${elemId}"]}"\"" >> ${file}
                        ...
                        echo "alias scenario-${elemId}=\"Execute scenarion ${elemId}\"" >> ${file}
                        echo "" >> ${file}
                    fi
                done
            fi
            ...
            if [[ "${FW_ELEMENT_SIT_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_SIT_LONG[@]}; do
                    if [[ "${FW_ELEMENT_SIT_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_SIT_LONG[\"${elemId}\"]=\""${FW_ELEMENT_SIT_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_SIT_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_SIT_DECMDS["${elemId}"]}"\"" >> ${file}
            ...
            if [[ "${FW_ELEMENT_TSK_LONG[*]}" != "" ]]; then
                for elemId in ${!FW_ELEMENT_TSK_LONG[@]}; do
                    if [[ "${FW_ELEMENT_TSK_DECMDS[${elemId}]}" == "${id}" ]]; then
                        echo "FW_ELEMENT_TSK_LONG[\"${elemId}\"]=\""${FW_ELEMENT_TSK_LONG["${elemId}"]}"\"" >> ${file}
                        echo "FW_ELEMENT_TSK_DECMDS[\"${elemId}\"]=\""${FW_ELEMENT_TSK_DECMDS["${elemId}"]}"\"" >> ${file}
                        ...
                        echo "FW_ELEMENT_TSK_SHOW_EXEC[\"${elemId}\"]=\""${FW_ELEMENT_TSK_PATH_TEXT["${elemId}"]}"\"" >> ${file}
        ...
        phases)
            ...
            if [[ -w "${file}" ]]; then
                rm ${file}; sedFile="${file}-sed"
                for array in FW_OBJECT_PHA_LONG FW_OBJECT_PHA_PRT_LVL FW_OBJECT_PHA_LOG_LVL FW_OBJECT_PHA_ERRCNT FW_OBJECT_PHA_WRNCNT FW_OBJECT_PHA_MSGCOD; do declare -p ${array} >> ${sedFile}; echo "" >> ${sedFile}; done
                sed -e "s/declare -A/declare -A -g/g" ${sedFile} > ${file}; rm ${sedFile}
            fi ;;
...
###ERROR MESSAGE
            ...
            themeFile="${FW_ELEMENT_MDS_PATH["${FW_OBJECT_THM_DECMDS[${id}]}"]}/themes/${id}.thm"

            rm ${file}
            echo "#!/usr/bin/env bash" >> ${file}
            printf "\n\n"  >> ${file}
            echo "##" >> ${file}
            echo "## Theme Declaration for theme ${id}" >> ${file}
            echo "## generated by skb-framework on $(date)" >> ${file}
            echo "##" >> ${file}
            printf "\n\n"  >> ${file}
            ...
            while read line; do
                case "${line}" in
                    "Set object themeitem "*)
                        ...
                        itemVal="${itemVal##*( )}"
                        shopt -u extglob
                        echo "FW_OBJECT_TIM_VAL[${itemId}]=${itemVal}"   >> ${file}
                        echo "    FW_OBJECT_TIM_SOURCE[${itemId}]=${id}" >> ${file}
                        count=$(( count + 1 )) ;;
                esac
            done < ${themeFile}
            printf "\n\n"  >> ${file} ;;
