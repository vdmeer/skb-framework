    ...
    shift; case "${cmd1}" in
        ...
        list)
            if [[ "${FW_ELEMENT_DIR_LONG[*]}" != "" ]]; then
                ...
                for id in "${keys[@]}"; do
                    printf "    %s (dec: %s / %s, set: %s)\n" "${id}"   "${FW_ELEMENT_DIR_DECMDS[${id}]}" "${FW_ELEMENT_DIR_DECPHA[${id}]}" "${FW_ELEMENT_DIR_PHA[${id}]}"
                    printf "        status:     s: %s, c: %s\n"         "${FW_ELEMENT_DIR_STATUS[${id}]}" "${FW_ELEMENT_DIR_STATUS_COMMENTS[${id}]}"
                    IFS=" " read -a numberArr <<< "${FW_ELEMENT_DIR_REQUESTED[${id}]}"; unset IFS
                    printf "        #req-in:    %s\n"                   "${#numberArr[@]}"
                    printf "        req-in:     %s\n"                   "${FW_ELEMENT_DIR_REQUESTED[${id}]}"
