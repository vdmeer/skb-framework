...
function __skb_Repeat_completions(){
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        ...
        *)  if [[ "${COMP_WORDS[COMP_CWORD-1]}" == "execution" ]]; then retval="1 2 3 4 5 6 7 8 9"
            ...
            elif [[ "${COMP_WORDS[COMP_CWORD-3]}" == "execution" ]]; then
                ...
                case "$COMP_LINE" in
                    "Repeat task execution "*)
                        taskName="${COMP_WORDS[5]//-/_}"
                        taskCompletion="__skb_task_${taskName}_words"
                        if [[ -n "$(type -t $taskCompletion)" && "$(type -t $taskCompletion)" = function ]]; then
                            retval="$(${taskCompletion})"
                        fi ;;
                esac
            fi ;;
    esac
...
complete -F __skb_Repeat_completions Repeat
