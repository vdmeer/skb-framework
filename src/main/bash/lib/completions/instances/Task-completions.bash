...
function __skb_Task_completions(){
    local retval=""
    case ${COMP_WORDS[COMP_CWORD-1]} in
        Task)           retval="$(Tasks has)" ;;

        requires)       retval="application dependency parameter directory-list directory file-list file task" ;;
        ...
        ## Task ID1 requires ...
        *)              if [[ "${COMP_WORDS[COMP_CWORD-2]}" == "Task" ]]; then
                            retval="requires"
                        fi ;;
    esac
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_Task_completions Task
