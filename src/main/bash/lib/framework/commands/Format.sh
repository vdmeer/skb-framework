...
##
## Format - command to format something
##
...
FW_TAGS_COMMANDS["Format"]="command to format something"

function Format() {
    if [[ -z "${1:-}" ]]; then Report process error "${FUNCNAME[0]}" E802 1 "$#"; return; fi
    local themeType id idLength leftMargin padding midString longest current width i char short value values defValues mode format
...
## TODO wrong error message, need too many
        ...
        current | element | table | tagline | themed)
...
## TODO wrong error message, need too many
                ...
                tagline-for)
                    ...
                    case "${cmd1}-${cmd2}-${cmd3}" in
                        ...
                        tagline-for-command | tagline-for-element | tagline-for-instance | tagline-for-object)
                            ...
                            case ${cmd3} in
                                ...
                                command)        Format themed text ${themeType}DescrFmt "${FW_TAGS_COMMANDS[${id}]}" ;;
