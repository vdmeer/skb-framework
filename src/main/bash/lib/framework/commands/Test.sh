...
FW_TAGS_COMMANDS["Test"]="command to test something"
...
function Test() {
    ...
    shift; case "${cmd1}" in
        ...
        configuration)
            ...
            id="${1}"; if [[ ! -n "${FW_OBJECT_CFG_LONG[${id}]:-}" ]]; then Report application error "${functionName}" "${cmdString1}" E804 "${cmd1}" "${id}"; return 1; fi ;;

        format)
            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString1}" E801 1 "$#"; return; fi
            id="${1}"; if [[ ! -n "${FW_OBJECT_FMT_LONG[${id}]:-}" ]]; then Report application error "${functionName}" "${cmdString1}" E804 "${cmd1}" "${id}"; return 1; fi ;;

        level)
            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString1}" E801 1 "$#"; return; fi
            id="${1}"; if [[ ! -n "${FW_OBJECT_LVL_LONG[${id}]:-}" ]]; then Report application error "${functionName}" "${cmdString1}" E804 "${cmd1}" "${id}"; return 1; fi ;;

        getopt)
            ! getopt --test > /dev/null; if [[ ${PIPESTATUS[0]} -ne 4 ]]; then Report application fatalerror "${functionName}" "${cmdString1}" E844; printf "\n"; Terminate 6; fi;;
        ...
        current | dir | element | error | existing | file | framework | fs | known | log | print | strict | used | warning)
            ...
            case "${cmd1}-${cmd2}" in
                ...
                current-mode)
                    ...
                    value="${1}"; if [[ ! -n "${FW_OBJECT_MOD_LONG[${value}]:-}" ]]; then Report application error "${functionName}" "${cmdString2}" E804 "${cmd2}" "${value}"; return 1; fi ;;
                current-phase)
                    if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString2}" E801 1 "$#"; return; fi
                    value="${1}"; if [[ ! -n "${FW_OBJECT_PHA_LONG[${value}]:-}" ]]; then Report application error "${functionName}" "${cmdString2}" E804 "${cmd2}" "${value}"; return 1; fi ;;
                current-theme)
                    if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString2}" E801 1 "$#"; return; fi
                    value="${1}"; if [[ ! -n "${FW_OBJECT_THM_LONG[${value}]:-}" ]]; then Report application error "${functionName}" "${cmdString2}" E804 "${cmd2}" "${value}"; return 1; fi ;;

                element-status)
                    if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString2}" E801 1 "$#"; return; fi
                    value="${1}"
                    case "${value}" in
                        S | s | success | E | e | errors | error | W | w | warnings | warning | N | n | not-attempted | not-tested) ;;
                        *)  Report application error "${functionName}" "${cmdString2}" E836 "${value}"
                            return 1 ;;
                    esac ;;

                framework-dependencies)
                    #ls pwd basename dircolors mktemp split head tail tsort wc who whoami
                    for index in cat cp cut date dirname mkdir mv rm rmdir sleep stat touch tr uname uniq sort; do
                        if [[ ! $(command -v ${index}) ]]; then Report application fatalerror "${functionName}" "${cmdString2}" E847 "${index}"; fi
                    done
                    if (( ${FW_OBJECT_SET_VAL["ERROR_COUNT"]} > 0 )); then printf "\n"; Terminate 10; fi
                    #curl pgrep pkill python3 tree
                    for index in bc less sed tput; do
                        if [[ ! $(command -v ${index}) ]]; then Report application fatalerror "${functionName}" "${cmdString2}" E848 "${index}"; fi
                    done
                    if (( ${FW_OBJECT_SET_VAL["ERROR_COUNT"]} > 0 )); then printf "\n"; Terminate 11; fi ;;
                ...
                strict-mode)
                    if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString2}" E801 1 "$#"; return; fi
                    value="${1}"; if [[ "${value}" != "on" && "${value}" != "off" ]]; then Report application error "${functionName}" "${cmdString2}" E804 "${cmd1} ${cmd2}" "${value}"; return 1; fi ;;

                error-count)
                    if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString2}" E801 1 "$#"; return; fi
                    value="${1}"; if [[ "${value}" =~ ^[+]?[0-9]+$ ]]; then return 0; else Report application error "${functionName}" "${cmdString2}" E882 "${value}"; return 1; fi ;;
                warning-count)
                    if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString2}" E801 1 "$#"; return; fi
                    value="${1}"; if [[ "${value}" =~ ^[+]?[0-9]+$ ]]; then return 0; else Report application error "${functionName}" "${cmdString2}" E883 "${value}"; return 1; fi ;;
                ...
                file-can | dir-can \
                    ...
                    case "${cmd1}-${cmd2}-${cmd3}" in
                        ...
                        file-can-create)
                            ...
                            if [[ ! -w "$(dirname ${value})" ]]; then Report application error "${functionName}" "${cmdString3}" E824 "file" "${valueText}"; return 1; fi ;;
                        file-can-delete)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E802 1 "$#"; return; fi
                            value="${1}"; valueText="${2:-$value}"
                            if [[ -f "${value}" && ! -w "${value}" ]]; then Report application error "${functionName}" "${cmdString3}" E825 "file" "${valueText}"; return 1; fi
                            if [[ ! -w "$(dirname ${value})" ]]; then Report application error "${functionName}" "${cmdString3}" E825 "file" "${valueText}"; return 1; fi ;;
                        dir-can-read)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E802 1 "$#"; return; fi
                            value="${1}"; valueText="${2:-$value}"
                        ...
                        dir-can-create)
                            ...
                            if [[ ! -w "$(dirname ${value})" ]]; then Report application error "${functionName}" "${cmdString3}" E824 "directory" "${valueText}"; return 1; fi ;;
                        dir-can-delete)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E802 1 "$#"; return; fi
                            value="${1}"; valueText="${2:-$value}"
                            ...
                            if [[ ! -w "$(dirname ${value})" ]]; then Report application error "${functionName}" "${cmdString3}" E825 "directory" "${valueText}"; return 1; fi ;;
                        ...
                        existing-application-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_APP_LONG[*]}" != "" && -n "${FW_ELEMENT_APP_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-dependency-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_DEP_LONG[*]}" != "" && -n "${FW_ELEMENT_DEP_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-module-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                        ...
                        existing-parameter-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_PAR_LONG[*]}" != "" && -n "${FW_ELEMENT_PAR_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-project-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_PRJ_LONG[*]}" != "" && -n "${FW_ELEMENT_PRJ_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-scenario-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_SCN_LONG[*]}" != "" && -n "${FW_ELEMENT_SCN_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-site-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_SIT_LONG[*]}" != "" && -n "${FW_ELEMENT_SIT_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-task-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_TSK_LONG[*]}" != "" && -n "${FW_ELEMENT_TSK_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        ...
                        existing-file-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_FIL_LONG[*]}" != "" && -n "${FW_ELEMENT_FIL_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-filelist-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_FLS_LONG[*]}" != "" && -n "${FW_ELEMENT_FLS_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-dir-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_DIR_LONG[*]}" != "" && -n "${FW_ELEMENT_DIR_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        existing-dirlist-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_DLS_LONG[*]}" != "" && -n "${FW_ELEMENT_DLS_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
                        ...
                        used-project-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_PRJ_LONG[*]}"  != "" && -n "${FW_ELEMENT_PRJ_LONG[${id}]:-}" ]];  then Report application error "${functionName}" "${cmdString3}" E807 "${cmd2}" "${id}"; return 1; fi ;;
                        used-scenario-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_SCN_LONG[*]}"  != "" && -n "${FW_ELEMENT_SCN_LONG[${id}]:-}" ]];  then Report application error "${functionName}" "${cmdString3}" E807 "${cmd2}" "${id}"; return 1; fi ;;
                        used-site-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_ELEMENT_SIT_LONG[*]}"  != "" && -n "${FW_ELEMENT_SIT_LONG[${id}]:-}" ]];  then Report application error "${functionName}" "${cmdString3}" E807 "${cmd2}" "${id}"; return 1; fi ;;
                        used-task-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
