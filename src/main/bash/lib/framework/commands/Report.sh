...
##
## Report - command to report something
##
...
FW_TAGS_COMMANDS["Report"]="command to report something"
...
#            FW_OBJECT_PHA_ERRCOD[${FW_OBJECT_SET_VAL["CURRENT_PHASE"]}]+=" ${1}"
#            Write fast config ;;
        ...
        process | application)
            ...
            case "${cmd1}-${cmd2}" in
                ...
                process-strictwarning | application-strictwarning \
                    ...
                    case ${cmd2} in
                        fatal | error)  FW_OBJECT_PHA_ERRCNT[${FW_OBJECT_SET_VAL["CURRENT_PHASE"]}]=$(( FW_OBJECT_PHA_ERRCNT[${FW_OBJECT_SET_VAL["CURRENT_PHASE"]}] + 1 ))
