    ...
    shift; case "${cmd1}" in
        ...
        current | dir | element | error | existing | file | framework | fs | known | log | print | strict | used | warning)
            ...
            case "${cmd1}-${cmd2}" in
                ...
                existing-exitcode | \
                existing-action | existing-element | existing-instance | existing-object | \
                ...
                file-can | dir-can \
                    ...
                    case "${cmd1}-${cmd2}-${cmd3}" in
                        ...
                        existing-exitcode-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ "${FW_INSTANCE_EXC_LONG[*]}" != "" && -n "${FW_INSTANCE_EXC_LONG[${id}]:-}" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;

                        existing-action-id | existing-element-id | existing-instance-id | existing-object-id)
                            if [[ "${#}" -lt 1 ]]; then Report process error "${FUNCNAME[0]}" "${cmdString3}" E801 1 "$#"; return; fi
                            id="${1}"
                            if [[ -f "${SF_HOME}/lib/framework/${cmd2}s/${id}.sh" ]]; then return; fi
                            Report application error "${functionName}" "${cmdString3}" E805 "${cmd2}" "${id}"; return 1 ;;
