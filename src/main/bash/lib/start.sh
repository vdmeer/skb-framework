...
if [[ -n "${SF_TEST_COMMANDS:-}" ]]; then
    if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] test \$SF_HOME\n"; fi
    ## Exitcode 4
    ...
    if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] test bash major\n"; fi
    ## Exitcode 5
    ...
    if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] test bash minor\n"; fi
    ## Exitcode 6
    ...
    if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] test getopt\n"; fi
    ## Exitcode 7
    ...
    if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] test coreutils\n"; fi
    ## Exitcode 8
    quit=false
    #pwd basename cp cut dircolors dirname mktemp mv split head stat tail tr tsort uniq wc who whoami
    for index in cat date ls mkdir rm rmdir sleep touch uname sort; do
    ...
    if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] test other commands\n"; fi
    ## Exitcode 9
    quit=false
...
if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[start] handing over to load\n"; fi
printf "\n"
#clear -x
...
if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "\n[cleanup] back from load, removing runtime configuration files\n"; fi
...
if [[ -n "${SF_DEBUG_START:-}" ]]; then printf "[end] finished, exiting\n"; fi
#exit 0
