...
function __skb_task_list_tasks_words(){
    local retval="" name
    local stdOptions="--describe --format --help --table --show-values"
    local filterOptions="--mode --origin --requested --status --tested --not-core"
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-tasks)  retval="${stdOptions} ${filterOptions} ${noneFilterOptions} ${onlyFilterOptions}" ;;
...
function __skb_task_list_tasks_completions(){
    local retval="$(__skb_task_list_tasks_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_tasks_completions list-tasks
