...
##
## list-scenarios - auto completion
##
...
function __skb_task_list_scenarios_words(){
    local retval=""
    local stdOptions="--describe --format --help --table --show-values"
    local filterOptions="--mode --origin --status --tested --not-core"
    local tableOptions="--without-status --with-legend --without-extras"

    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-scenarios)  retval="${stdOptions} ${filterOptions}" ;;
...
function __skb_task_list_scenarios_completions(){
    local retval="$(__skb_task_list_scenarios_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_scenarios_completions list-scenarios
