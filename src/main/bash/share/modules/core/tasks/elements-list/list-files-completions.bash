...
function __skb_task_list_files_words(){
    local retval=""
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-files)  retval="${stdOptions} ${taskOptions}" ;;
...
function __skb_task_list_files_completions(){
    local retval="$(__skb_task_list_files_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_files_completions list-files
