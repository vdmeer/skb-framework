...
##
## list-dirlists - task that lists dirlists
##
...
Clioptions add option filter-origin    "directory lists"
Clioptions add option filter-requested "directory lists"
Clioptions add option filter-status    "directory lists"
Clioptions add option filter-tested    "directory lists"
Clioptions add option filter-not-core  "directory lists"
...
for id in $arr; do
    if [[ -n "${origin}" ]]; then
        if [[ "${origin}" != "${FW_ELEMENT_DLS_DECMDS[${id}]}" ]]; then
            remove+=" "$id
        fi
    fi
    if [[ -n "${requested}" ]]; then
        case "${requested}" in
            y)  if [[ ! -n "${FW_ELEMENT_DLS_REQUESTED[${id}]:-}" ]]; then remove+=" "$id; fi ;;
            n)  if [[   -n "${FW_ELEMENT_DLS_REQUESTED[${id}]:-}" ]]; then remove+=" "$id; fi ;;
        esac
    fi
    if [[ -n "${status}" ]]; then
        if [[ "${status}" != "${FW_ELEMENT_DLS_STATUS[${id}]}" ]]; then
            remove+=" "$id
        fi
    fi
    if [[ -n "${tested}" ]]; then
        case "${tested}" in
            y)  if [[ "${FW_ELEMENT_DLS_STATUS[${id}]}" == "N" ]]; then remove+=" "$id; fi ;;
            n)  if [[ "${FW_ELEMENT_DLS_STATUS[${id}]}" != "N" ]]; then remove+=" "$id; fi ;;
        esac
    fi
    if [[ "${not_core}" == yes ]];      then if [[ "Core" == "${FW_ELEMENT_DLS_DECMDS[${id}]}" ]]; then remove+=" "$id; fi; fi
done
...
############################################################################################
##
## print dirlists as list or table
##
############################################################################################
if [[ "${FW_INSTANCE_CLI_SET["table"]}" == "no" ]]; then
    printf "\n  "
    Format themed text listHeadFmt "Directory Lists"
    printf "\n"
    Print dirlist list "$arr" ${showValues}
else
    Print dirlist table "${arr}" ${showValues} ${withLegend} ${withoutStatus} ${withoutExtras}
fi
