...
function __skb_task_list_dirs_words(){
    local retval=""
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-dirs)  retval="${stdOptions} ${taskOptions}" ;;
...
function __skb_task_list_dirs_completions(){
    local retval="$(__skb_task_list_dirs_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_dirs_completions list-dirs
