...
##
## list-filelists - task that lists filelists
##
...
Clioptions add option filter-origin    "file lists"
Clioptions add option filter-requested "file lists"
Clioptions add option filter-status    "file lists"
Clioptions add option filter-tested    "file lists"
Clioptions add option filter-not-core  "file lists"
...
for id in $arr; do
    if [[ -n "${origin}" ]]; then
        if [[ "${origin}" != "${FW_ELEMENT_FLS_DECMDS[${id}]}" ]]; then
            remove+=" "$id
        fi
    fi
    if [[ -n "${requested}" ]]; then
        case "${requested}" in
            y)  if [[ ! -n "${FW_ELEMENT_FLS_REQIN[${id}]:-}" ]]; then remove+=" "$id; fi ;;
            n)  if [[   -n "${FW_ELEMENT_FLS_REQIN[${id}]:-}" ]]; then remove+=" "$id; fi ;;
        esac
    fi
    if [[ -n "${status}" ]]; then
    ...
    if [[ "${not_core}" == yes ]];      then if [[ "Core" == "${FW_ELEMENT_FLS_DECMDS[${id}]}" ]]; then remove+=" "$id; fi; fi
done
...
############################################################################################
##
## print filelists as list or table
##
############################################################################################
if [[ "${FW_INSTANCE_CLI_SET["table"]}" == "no" ]]; then
    printf "\n  "
    Format themed text listHeadFmt "File Lists"
    printf "\n"
    Print filelist list "$arr" ${showValues}
else
    Print filelist table "${arr}" ${showValues} ${withLegend} ${withoutStatus} ${withoutExtras}
fi
