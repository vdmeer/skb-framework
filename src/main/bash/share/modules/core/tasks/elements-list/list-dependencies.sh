...
##
## list-dependencies - task that lists dependencies
##
...
for id in $arr; do
    if [[ -n "${origin}" ]]; then
        if [[ "${origin}" != "${FW_ELEMENT_DEP_DECMDS[${id}]}" ]]; then
            remove+=" "$id
        fi
    fi
    if [[ -n "${requested}" ]]; then
    ...
    if [[ "${not_core}" == yes ]];      then if [[ "Core" == "${FW_ELEMENT_DEP_DECMDS[${id}]}" ]]; then remove+=" "$id; fi; fi
done
...
############################################################################################
##
## print dependencies as list or table
##
############################################################################################
if [[ "${FW_INSTANCE_CLI_SET["table"]}" == "no" ]]; then
    printf "\n  "
    Format themed text listHeadFmt Dependencies
    printf "\n"
    Print dependency list "$arr" ${showValues}
else
    Print dependency table "${arr}" ${showValues} ${withLegend} ${withoutStatus} ${withoutExtras}
fi
