...
##
## list-dirs - task that lists dirs
##
...
Clioptions add option filter-origin    directories
Clioptions add option filter-requested directories
Clioptions add option filter-status    directories
Clioptions add option filter-tested    directories
Clioptions add option filter-not-core  directories
...
for id in $arr; do
    ...
    if [[ -n "${requested}" ]]; then
        case "${requested}" in
            y)  if [[ ! -n "${FW_ELEMENT_DIR_REQUESTED[${id}]:-}" ]]; then remove+=" "$id; fi ;;
            n)  if [[   -n "${FW_ELEMENT_DIR_REQUESTED[${id}]:-}" ]]; then remove+=" "$id; fi ;;
        esac
    fi
    if [[ -n "${status}" ]]; then
        if [[ "${status}" != "${FW_ELEMENT_DIR_STATUS[${id}]}" ]]; then
            remove+=" "$id
        fi
    fi
    if [[ -n "${tested}" ]]; then
        case "${tested}" in
            y)  if [[ "${FW_ELEMENT_DIR_STATUS[${id}]}" == "N" ]]; then remove+=" "$id; fi ;;
...
############################################################################################
##
## print dirs as list or table
##
############################################################################################
if [[ "${FW_INSTANCE_CLI_SET["table"]}" == "no" ]]; then
    printf "\n  "
    Format themed text listHeadFmt Directories
    printf "\n"
    Print dir list "$arr" ${showValues}
else
    Print dir table "${arr}" ${showValues} ${withLegend} ${withoutStatus} ${withoutExtras}
fi
