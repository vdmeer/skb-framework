...
##
## list-applications - auto completion
##
...
function __skb_task_list_applications_words(){
    local retval=""
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-applications)  retval="${stdOptions} ${taskOptions}" ;;
...
function __skb_task_list_applications_completions(){
    local retval="$(__skb_task_list_applications_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_applications_completions list-applications
