...
function __skb_task_list_projects_words(){
    local retval=""
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-projects)  retval="${stdOptions} ${filterOptions}" ;;
...
function __skb_task_list_projects_completions(){
    local retval="$(__skb_task_list_projects_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_projects_completions list-projects
