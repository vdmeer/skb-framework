...
##
## list-configuration - auto completion
##
...
function __skb_task_list_configuration_words(){
    local retval=""
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-configuration)  retval="${stdOptions} ${taskOptions}" ;;
...
function __skb_task_list_configuration_completions(){
    local retval="$(__skb_task_list_configuration_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_configuration_completions list-configuration
