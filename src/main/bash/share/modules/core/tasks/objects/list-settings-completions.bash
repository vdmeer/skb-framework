...
function __skb_task_list_settings_words(){
    local retval=""
    local stdOptions="--describe --format --help --table  --show-values"
    local taskOptions="--cli --default --env --file --framework --project --scenario --shell --task"
    local tableOptions="--without-status --with-legend --without-extras"

    case ${COMP_WORDS[COMP_CWORD-1]} in
        list-settings)  retval="${stdOptions} ${taskOptions}" ;;
        ...
        *)  retval="${stdOptions} ${taskOptions}"
            case "$COMP_LINE" in
                ...
                *"--cli"*)              retval="${retval//"--clie"/}" ;;&
                *"--default"*)          retval="${retval//"--default"/}" ;;&
                *"--env"*)              retval="${retval//"--env"/}" ;;&
                *"--file"*)             retval="${retval//"--file"/}" ;;&
                *"--framework"*)        retval="${retval//"--framework"/}" ;;&
                *"--project"*)          retval="${retval//"--project"/}" ;;&
                *"--scenario"*)         retval="${retval//"--scenario"/}" ;;&
                *"--shell"*)            retval="${retval//"--shell"/}" ;;&
                *"--task"*)             retval="${retval//"--task"/}" ;;
            esac
            ;;
    esac
    printf "%s" "${retval}"
...
function __skb_task_list_settings_completions(){
    local retval="$(__skb_task_list_settings_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_list_settings_completions list-settings
