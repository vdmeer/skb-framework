...
##
## describe-elements - auto completion
##
...
function __skb_task_describe_elements_words(){
    local retval=""
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        describe-elements)  retval="${stdOptions} ${taskOptions}" ;;
...
function __skb_task_describe_elements_completions(){
    local retval="$(__skb_task_describe_elements_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_describe_elements_completions describe-elements
