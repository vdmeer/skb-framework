...
function __skb_task_debug_tasks_words(){
    local retval="" name
    ...
    case ${COMP_WORDS[COMP_CWORD-1]} in
        debug-tasks)  retval="${stdOptions} ${filterOptions} ${noneFilterOptions} ${onlyFilterOptions}" ;;
...
function __skb_task_debug_tasks_completions(){
    local retval="$(__skb_task_debug_tasks_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_debug_tasks_completions debug-tasks
