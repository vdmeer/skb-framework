...
##
## describe-applications - task that describes applications
##
...
Clioptions add option filter-id        describe application
...
if [[ "${FW_INSTANCE_CLI_SET["id"]}" == "yes" ]]; then
    Test existing application id "${FW_INSTANCE_CLI_VAL["id"]}"
...
if [[ -n "${name}" ]]; then
    ...
    for id in $arr; do
        if [[ -n "${origin}" ]]; then
            if [[ "${origin}" != "${FW_ELEMENT_APP_DECMDS[${id}]}" ]]; then
                remove+=" "$id
            fi
        fi
        if [[ -n "${requested}" ]]; then
        ...
        if [[ "${not_core}" == yes ]];      then if [[ "Core" == "${FW_ELEMENT_APP_DECMDS[${id}]}" ]]; then remove+=" "$id; fi; fi
    done
    for id in $remove; do
        arr=${arr/${id}/}
    done
...
############################################################################################
##
## print application descriptions
##
############################################################################################
