...
##
## describe-scenarios - auto completion
##
...
function __skb_task_describe_scenarios_words(){
    local retval=""
    local stdOptions="--describe --format --help"
    local taskOptions="--id --mode --origin --status --tested --not-core"

    case ${COMP_WORDS[COMP_CWORD-1]} in
        describe-scenarios)  retval="${stdOptions} ${taskOptions}" ;;
...
function __skb_task_describe_scenarios_completions(){
    local retval="$(__skb_task_describe_scenarios_words)"
    if [[ -n "${retval}" ]]; then COMPREPLY=($(compgen -W "${retval}" -- "${COMP_WORDS[COMP_CWORD]}")); fi
}
complete -F __skb_task_describe_scenarios_completions describe-scenarios
