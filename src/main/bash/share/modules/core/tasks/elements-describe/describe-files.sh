...
##
## describe-files - task that describes files
##
...
Clioptions add option filter-id        describe file
...
if [[ -n "${name}" ]]; then
    ...
    for id in $arr; do
        if [[ -n "${origin}" ]]; then
            if [[ "${origin}" != "${FW_ELEMENT_FIL_DECMDS[${id}]}" ]]; then
                remove+=" "$id
            fi
        fi
        if [[ -n "${requested}" ]]; then
            case "${requested}" in
                y)  if [[ ! -n "${FW_ELEMENT_FIL_REQIN[${id}]:-}" ]]; then remove+=" "$id; fi ;;
                n)  if [[   -n "${FW_ELEMENT_FIL_REQIN[${id}]:-}" ]]; then remove+=" "$id; fi ;;
            esac
        fi
        if [[ -n "${status}" ]]; then
        ...
        if [[ "${not_core}" == yes ]];      then if [[ "Core" == "${FW_ELEMENT_FIL_DECMDS[${id}]}" ]]; then remove+=" "$id; fi; fi
    done
    for id in $remove; do
        arr=${arr/${id}/}
    done
