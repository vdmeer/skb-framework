...
== make-target-sets
This task executes one or more targets out of a defined set of targets.
...
It can even build targets from multiple target sets in the same execution, as long as the actual targets are supported by all target set.


The parameter _MAKE_TARGET_SETS_ is used to identify the target sets that can be used.
...
There can only be one target set in any of the directories of _MAKE_TARGET_SETS_.

The general options, shown below, allow to direct the behavior of the task.

* _build_ - request one or more target sets to be built.
...
----
include::{skb-site-target-dir}/adoc-screens/make-target-sets--help--pdf.adoc[tags=options]
----
endif::[]
ifdef::backend-html5[]
[source%nowrap,bash,indent=0,subs="attributes,quotes"]
----
include::{skb-site-target-dir}/adoc-screens/make-target-sets--help--adoc.adoc[tags=options]
----
endif::[]

=== Target Option
The task can builds any target requested.
Targets are declared in the file `skb-ts.id` per target set.

ifdef::backend-pdf[]
[source%nowrap,bash,indent=0,subs="attributes"]
----
include::{skb-site-target-dir}/adoc-screens/make-target-sets--help--pdf.adoc[tags=target-option]
----
endif::[]
ifdef::backend-html5[]
[source%nowrap,bash,indent=0,subs="attributes,quotes"]
----
include::{skb-site-target-dir}/adoc-screens/make-target-sets--help--adoc.adoc[tags=target-option]
----
endif::[]

There is no default target.
However, the standard taget _all_ will build all targets in the declared sequence.



=== Filters
Filters allow to specify which of the loaded (and available) target sets should be built:

* _all_ - will build all target sets, in the oder they are listed (alphabetically)
* _id_ - can be used to build a specific target set

ifdef::backend-pdf[]
[source%nowrap,bash,indent=0,subs="attributes"]
----
include::{skb-site-target-dir}/adoc-screens/make-target-sets--help--pdf.adoc[tags=filters]
----
endif::[]
ifdef::backend-html5[]
[source%nowrap,bash,indent=0,subs="attributes,quotes"]
----
include::{skb-site-target-dir}/adoc-screens/make-target-sets--help--adoc.adoc[tags=filters]
----
endif::[]
...
----
  Target Sets
    - skb-fw - targets to build the SKB-Framework distribution/site artifacts - SOME_PATH
      --> with target(s): help clean tool mkdirs readme dist adocindex documents site
----
...
[source%nowrap,bash,indent=0]
----
make-target-sets --id skb-fw --targets help
----
...
----
include::{skb-site-target-dir}/adoc-screens/ts-fw-help.txt[]
----
...
=== Target Set Files
As mentioned above, a target set suitable for this task should provide a file with its metadata and might provide a file with scripts to be used in the build.
Both files must be located in the same directory and the directory must be named in _MAKE_TARGET_SETS_.
...
* An identifier.
    ...
    The identifier must be unique for all target sets in _MAKE_TARGET_SETS_.
* A description.
...
=== Requirements
The task takes the sites it should load and build from the parameter _MAKE_TARGET_SETS_.
