...
[source%nowrap,bash,indent=0,subs="attributes,quotes"]
----
        *--adoc*        primary target: aggregated ADOC file
        *--html*        secondary target: HTML file
        *--manp*        secondary target: man page file
        *--pdf*         secondary target: PDF file)
        *--text*        secondary target: text versions: ansi, text, text-anon
...
[source%nowrap,bash,indent=0,subs="attributes,quotes"]
----
   *-i* | *--install*     only list 'install' app mode flavor tasks and scenarios
