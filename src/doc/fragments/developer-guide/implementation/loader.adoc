...
To makes things simple, the provided `_include` files are used to load all API functions.
Error and warning counters are reset, i.e. set to _0_.
...
Now the declarations of shell commands (lines 1-7) and exit codes (lines 8-14) can be loaded, either from cache or source.
Errors here indicate a framework bug or runtime problem, since commands and exit codes are core features and rather static.
