...
== ErrorcodeTagline()
_void ErrorcodeTagline( $1, $2, [$3] )_ - prints the error code tagline with formatting (padding).

This function prints the error code tag line (the one line or short description) with padding after the text.
The overall length is taken from _CONSOLE_MAP["EC_DESCRIPTION_LENGTH"]_.
The line is printed as follows:

** If the tag line for the error code is smaller (less characters) than the overall length, then
...
* `$1`, _string_ - the error code identifier.
...
* For unknown error code identifiers (names)
...
The examples above call the function for the error code _012_.
