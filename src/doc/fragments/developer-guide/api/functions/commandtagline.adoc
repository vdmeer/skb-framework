...
== CommandTagline()
_void CommandTagline( $1, $2, [$3] )_ - prints the command tagline with formatting (padding).

This function prints the command tag line (the one line or short description) with padding after the text.
The overall length is taken from _CONSOLE_MAP["CMD_DESCRIPTION_LENGTH"]_.
The line is printed as follows:

** If the tag line for the command is smaller (less characters) than the overall length, then
...
* `$1`, _string_ - the command identifier, long or short form.
...
* For unknown command identifiers (names)
...
The examples above call the function for the command _history_.
...
----
show history or run history command
show history or run history command⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵
----
