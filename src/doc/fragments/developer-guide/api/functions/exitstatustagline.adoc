...
== ExitstatusTagline()
_void ExitstatusTagline( $1, $2, [$3] )_ - prints the exit status tagline with formatting (padding).

This function prints the exit status tag line (the one line or short description) with padding after the text.
The overall length is taken from _CONSOLE_MAP["ES_DESCRIPTION_LENGTH"]_.
The line is printed as follows:

** If the tag line for the exist status is smaller (less characters) than the overall length, then
...
* `$1`, _string_ - the exit status identifier.
...
* For unknown exit status identifiers (names)
...
----
ExitstatusTagline 012 3 none
ExitstatusTagline 012
----

The examples above call the function for the exit status _012_.
