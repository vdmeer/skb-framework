...
== OptionTagline()
_void OptionTagline( $1, $2, [$3] )_ - prints the option tagline with formatting (padding).

This function prints the option tag line (the one line or short description) with padding after the text.
The overall length is taken from _CONSOLE_MAP["OPT_DESCRIPTION_LENGTH"]_.
The line is printed as follows:

** If the tag line for the option is smaller (less characters) than the overall length, then
...
* `$1`, _string_ - the option identifier, long or short form.
...
The examples above call the function for the option _install_.
...
[source%nowrap]
----
activate 'install' flavor, loads special tasks
activate 'install' flavor, loads special tasks⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵⎵
----
endif::[]
ifdef::backend-html5[]
