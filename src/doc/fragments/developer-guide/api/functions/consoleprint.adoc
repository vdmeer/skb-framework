...
[source%nowrap]
----
  -> [Fatal] internal error: no flavor set
  -> [Error] declare command - could not declare command
  -> [Warning/strict] process-task/param - start-browser  with unset parameter 'BROWSER'
  -> [Warning] overwriting list-tasks
  --> [Info] cleaning cache and exit
    > declared command cls
      >    manual - task opt start-pdf-viewer
done
----
endif::[]
ifdef::backend-html5[]
