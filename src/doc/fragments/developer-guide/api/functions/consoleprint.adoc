...
The type if the message can be one of

* _fatal_ for fatal errors, i.e. errors that prevent the framework from running any further.
...
* _warn-strict_ for issues that a treated as errors in _strict_ mode but only as warnings if _stric_ mode is not set.
...
* _warn_ - for warnings of any kind (other than strict warnings as describe above).
...
* _info_ - for tagged information (for information without a tag see _message_).
...
* _message_ - any kind of message that should be printed.
    The message in `$2` will simply be printed.
    _GetQuietl()_ is used to determine if messages should be printed (`on`) or not (`off`).
...
* {blank}
include::{skb-site-main-dir}/doc/fragments/developer-guide/api/function-links/getlevel-link.adoc[]
to get the right level.
* {blank}
include::{skb-site-main-dir}/doc/fragments/developer-guide/api/function-links/getquiet-link.adoc[]
to get the right quiet setting.
...
----
  ...
  --> [<span style="color: #0000FF">Info</span>] cleaning cache and exit
