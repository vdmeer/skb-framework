...
== DescribeScenario()
_void DescribeScenario( $1, $2, $3, [$4])_ - describes a scenario with various options.

This function describes the scenario _$1_ using the print option _$2_, the print feature(s) _$3_, and an optional print mode _$4_.
...
* `$2`, _string_ - the print option, one of:
    ** _standard_ - prints one line for the scenario: short form (if declared), long form, tag line.
    ** _full* - prints _standard_ plus the scenario description.
* `$3`, _string_ - the print features.
    ...
    ** `adoc` - prints a description in _adoc_ taken from the scenario declaration.
    ** `ansi` -  prints a description with plain text formatting taken from scenario declaration.
    ** `text` - actually any string starting with `text` - prints a description with plain text formatting taken from scenario declaration.
...
* For an unknown scenario identifier (name)
...
The example above calls the function to get a full description of the scenario _ test-list-tasks_,
...
----
      *test-list-tasks*, *tlt* - runs list tasks with various options
        This is a test scenario for _list_ tasks. It runs all those tasks with
        various options.
----
