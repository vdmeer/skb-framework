...
== DescribeExitstatus()
_void DescribeExitstatus( $1, $2, $3, [$4])_ - describes an exit status with various options.

This function describes the exit status _$1_ using the print option _$2_, the print feature(s) _$3_, and an optional print mode _$4_.


*Parameter*

* `$1`, _string_ - the exit status identifier to process, long- or short form
* `$2`, _string_ - the print option, one of:
    ** _standard_ - prints one line for the exit status: number and tag line.
    ** _full* - prints _standard_ plus the exit status description.
* `$3`, _string_ - the print features.
    ...
    ** `adoc` - prints a description in _adoc_ taken from the exit status declaration.
    ** `ansi` -  prints a description with plain text formatting taken from exit status declaration.
    ** `text` - actually any string starting with `text` - prints a description with plain text formatting taken from exit status declaration.
...
* For an unknown exit status identifier (name)
...
----
DescribeExitstatus 010 full "ansi line-indent" ansi
----

The example above calls the function to get a full description of the exit status _010_,
